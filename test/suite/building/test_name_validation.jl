...
function test_name_validation()
    Test.@testset "Name Validation Tests" verbose=VERBOSE showtiming=SHOWTIMING begin
        ...
        Test.@testset "__collect_used_names" begin
            ...
            names = CTModels.Building.__collect_used_names(ocp)
        ...
        Test.@testset "__has_name_conflict" begin
            ...
            ocp = CTModels.PreModel()
            Test.@test !CTModels.Building.__has_name_conflict(ocp, "x")
            Test.@test !CTModels.Building.__has_name_conflict(ocp, "y")

            # With state - conflicts with state names
            ocp = CTModels.PreModel()
            CTModels.state!(ocp, 2, "x", ["x₁", "x₂"])
            Test.@test CTModels.Building.__has_name_conflict(ocp, "x")      # conflicts with state name
            Test.@test CTModels.Building.__has_name_conflict(ocp, "x₁")     # conflicts with state component
            Test.@test CTModels.Building.__has_name_conflict(ocp, "x₂")     # conflicts with state component
            Test.@test !CTModels.Building.__has_name_conflict(ocp, "y")     # no conflict

            # With exclude_component
            ocp = CTModels.PreModel()
            CTModels.state!(ocp, 2, "x", ["x₁", "x₂"])
            Test.@test !CTModels.Building.__has_name_conflict(ocp, "x", :state)      # exclude state names
            Test.@test !CTModels.Building.__has_name_conflict(ocp, "x₁", :state)     # exclude state components
            Test.@test !CTModels.Building.__has_name_conflict(ocp, "x₂", :state)     # exclude state components
            ...
            Test.@test CTModels.Building.__has_name_conflict(ocp, "x")      # conflicts with state
            Test.@test CTModels.Building.__has_name_conflict(ocp, "u")      # conflicts with control
            Test.@test CTModels.Building.__has_name_conflict(ocp, "t")      # conflicts with time
            Test.@test CTModels.Building.__has_name_conflict(ocp, "x₁")     # conflicts with state component
            Test.@test !CTModels.Building.__has_name_conflict(ocp, "y")     # no conflict

            # Test exclude_component with multiple components
            Test.@test !CTModels.Building.__has_name_conflict(ocp, "x", :state)      # exclude state
            Test.@test CTModels.Building.__has_name_conflict(ocp, "x", :control)     # still conflicts (x is state name, not excluded)
            Test.@test !CTModels.Building.__has_name_conflict(ocp, "u", :control)    # exclude control
            Test.@test !CTModels.Building.__has_name_conflict(ocp, "t", :time)       # exclude time
        end

        Test.@testset "__validate_name_uniqueness" begin
            # Valid case - empty model
            ocp = CTModels.PreModel()
            Test.@test_throws Exceptions.IncorrectArgument CTModels.Building.__validate_name_uniqueness(
                ocp, "", ["x"], :state
            )
            ...
            ocp = CTModels.PreModel()
            Test.@test_nowarn CTModels.Building.__validate_name_uniqueness(
                ocp, "x", ["x"], :state
            )
