...
function test_name_validation()
    Test.@testset "Name Validation Helpers" verbose = VERBOSE showtiming = SHOWTIMING begin
        ...
        @testset "__validate_name_uniqueness" begin
            ...
            ocp = CTModels.PreModel()
            @test_throws CTBase.IncorrectArgument CTModels.OCP.__validate_name_uniqueness(ocp, "", ["x"], :state)
            ...
            ocp = CTModels.PreModel()
            @test_throws CTBase.IncorrectArgument CTModels.OCP.__validate_name_uniqueness(ocp, "x", ["x", "y"], :state)
            ...
            ocp = CTModels.PreModel()
            @test_throws CTBase.IncorrectArgument CTModels.OCP.__validate_name_uniqueness(ocp, "x", ["y", "y"], :state)
            
            # Error: conflict with existing names
            ocp = CTModels.PreModel()
            CTModels.control!(ocp, 1, "u")
            @test_throws CTBase.IncorrectArgument CTModels.OCP.__validate_name_uniqueness(ocp, "u", ["x₁"], :state)  # name conflicts
            @test_throws CTBase.IncorrectArgument CTModels.OCP.__validate_name_uniqueness(ocp, "x", ["u"], :state)  # component conflicts
            ...
            @test_throws CTBase.IncorrectArgument CTModels.OCP.__validate_name_uniqueness(ocp, "t", ["y₁"], :state)  # conflicts with time
            @test_throws CTBase.IncorrectArgument CTModels.OCP.__validate_name_uniqueness(ocp, "x", ["y₁"], :control)  # conflicts with state
            @test_throws CTBase.IncorrectArgument CTModels.OCP.__validate_name_uniqueness(ocp, "u", ["y₁"], :variable)  # conflicts with control
            @test_throws CTBase.IncorrectArgument CTModels.OCP.__validate_name_uniqueness(ocp, "v", ["y₁"], :state)  # conflicts with variable
            @test_throws CTBase.IncorrectArgument CTModels.OCP.__validate_name_uniqueness(ocp, "x", ["x₁"], :control)  # conflicts with state component
            @test_throws CTBase.IncorrectArgument CTModels.OCP.__validate_name_uniqueness(ocp, "x₁", ["y"], :control)  # conflicts with state component
