...
function test_times()
    Test.@testset "times" verbose = VERBOSE showtiming = SHOWTIMING begin
        ...
        @test CTModels.name(time) == "s"
        @test_throws Exceptions.IncorrectArgument CTModels.time(time, Float64[])
        ...
        @test_throws Exceptions.PreconditionError CTModels.time!(ocp, t0=0.0, tf=10.0)

        # if ind0 or indf is provided, the variable must be set
        ocp = CTModels.PreModel()
        @test_throws Exceptions.PreconditionError CTModels.time!(ocp, ind0=1, tf=10.0)
        @test_throws Exceptions.PreconditionError CTModels.time!(ocp, t0=0.0, indf=1)
        @test_throws Exceptions.PreconditionError CTModels.time!(ocp, ind0=1, indf=2)

        # index must satisfy 1 <= index <= q
        ocp = CTModels.PreModel()
        CTModels.variable!(ocp, 2)
        @test_throws Exceptions.IncorrectArgument CTModels.time!(ocp, ind0=0, tf=10.0)
        @test_throws Exceptions.IncorrectArgument CTModels.time!(ocp, ind0=3, tf=10.0)
        @test_throws Exceptions.IncorrectArgument CTModels.time!(ocp, t0=0.0, indf=0)
        @test_throws Exceptions.IncorrectArgument CTModels.time!(ocp, t0=0.0, indf=3)
        @test_throws Exceptions.IncorrectArgument CTModels.time!(ocp, ind0=0, indf=3)
        @test_throws Exceptions.IncorrectArgument CTModels.time!(ocp, ind0=3, indf=3)
        ...
        Test.@testset "times: Temporal validation" verbose = VERBOSE showtiming = SHOWTIMING begin
            # t0 > tf
            ocp = CTModels.PreModel()
            @test_throws Exceptions.IncorrectArgument CTModels.time!(ocp, t0=1.0, tf=0.0)

            # t0 = tf
            ocp = CTModels.PreModel()
            @test_throws Exceptions.IncorrectArgument CTModels.time!(ocp, t0=1.0, tf=1.0)
