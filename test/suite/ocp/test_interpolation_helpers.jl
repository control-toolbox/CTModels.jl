...
using CTModels
using CTModels.OCP:
    build_interpolated_function, _interpolate_from_data, _wrap_scalar_and_deepcopy
...
function test_interpolation_helpers()
    @testset "Interpolation Helpers" verbose = VERBOSE showtiming = SHOWTIMING begin
        ...
        @testset "_interpolate_from_data: dimension validation" begin
            ...
            func = _interpolate_from_data(X_2d, T, 2, Matrix{Float64}; expected_dim=2)
            @test !isnothing(func)

            # Valid: matrix has 2 columns, we extract 1
            func = _interpolate_from_data(X_2d, T, 1, Matrix{Float64}; expected_dim=1)
            @test !isnothing(func)
        ...
        @testset "build_interpolated_function: optional duals" begin
            ...
            fdual = build_interpolated_function(nothing, T, 2, Nothing; allow_nothing=true)
