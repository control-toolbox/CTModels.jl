...
__variable_components(q::Dimension, name::String)::Vector{String} =
    q > 1 ? [name * CTBase.ctindices(i) for i in range(1, q)] : [name]
...
(
    variable(model::OptimalControlModel{T,S,C,V,D,O})::V
) where {
...
variable_components(model::OptimalControlModel)::Vector{String} =
    components(variable(model))
