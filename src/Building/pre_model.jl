...
building it into an immutable [`CTModels.Models.Model`](@extref). Fields can be set in any order
...
- `definition::AbstractDefinition`: Symbolic definition; defaults to
  [`CTModels.Components.EmptyDefinition`](@extref) and becomes a [`CTModels.Components.Definition`](@extref) when
  [`CTModels.Building.definition!`](@extref) is called with a real expression.
...
Pack local variables into a [`PreModel`](@extref) object.

Generated by `Parameters.@with_kw` for the mutable `PreModel` type.

# Example
```julia-repl
julia> pre = PreModel(state=...)
julia> @pack_PreModel pre = state, control
```

See also: [`@unpack_PreModel`](@extref), [`@pack_PreModel!`](@extref).
"""
:var"@pack_PreModel"
...
Pack local variables into an existing [`PreModel`](@extref) object in-place.

Generated by `Parameters.@with_kw` for the mutable `PreModel` type.

See also: [`@pack_PreModel`](@extref), [`@unpack_PreModel`](@extref).
"""
:var"@pack_PreModel!"
...
Unpack fields from a [`PreModel`](@extref) object into local variables.
...
```julia-repl
julia> pre = PreModel()
julia> @unpack_PreModel pre = state, control
```

See also: [`@pack_PreModel`](@extref).
"""
:var"@unpack_PreModel"
...
Return `true` if `d` is an [`CTModels.Components.EmptyDefinition`](@extref).
...
Return `true` if the definition field of the `PreModel` is an [`CTModels.Components.EmptyDefinition`](@extref).
...
`control` defaults to [`CTModels.Components.EmptyControlModel`](@extref) (dimension `0`),
so — unlike [`CTModels.Models.state_dimension`](@extref) — no precondition on `control` having been set
is required.
...
`variable` defaults to [`CTModels.Components.EmptyVariableModel`](@extref) (dimension `0`),
so — unlike [`CTModels.Models.state_dimension`](@extref) — no precondition on `variable` having been set
is required.
