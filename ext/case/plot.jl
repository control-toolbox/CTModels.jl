...
#
# The thin public methods forward to `_plot` / `_plot!`, which build the
# backend-agnostic figure with `CTModels.PlotCase.build_figure` and render it
# through the `CTBase.Plotting` Plots backend. Everything domain-specific (the
# vocabulary, panels, decorations, layout template) lives in `CTModels.PlotCase`.
#
